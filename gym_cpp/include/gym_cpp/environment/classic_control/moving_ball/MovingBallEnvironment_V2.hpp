...
namespace gym{
  class MovingBallEnvironment_V2:public Environment{
    public:
      MovingBallEnvironment_V2(int stateHistoryLength = 1 , double maxAction=5.0, double maxPosition = 2.0,double maxSpeed = 5.0,double timeStep = 0.01):
        Environment()
      {
      ...
      bool step(Eigen::VectorXd& newState, double& reward, Eigen::VectorXd u) override{
        ...
        // TODO: this shouldn't reseted
        newState = Eigen::VectorXd::Zero(2*stateHistoryLength_);
        for(int i=0;i<stateHistoryLength_-1;i++){
          newState.segment(2*i,2) = newState.segment(2*(i+1),2);

        }
        newState.segment(2*(stateHistoryLength_-1),2) = desiredPosition_-position_;
      ...
      Eigen::VectorXd reset() override{
        ...
        Eigen::VectorXd state = Eigen::VectorXd::Zero(2*stateHistoryLength_);
        for(int i=0;i<stateHistoryLength_;i++){
          state.segment(2*i,2) =  desiredPosition_-position_;
