...
namespace gym{
  class MovingBallEnvironment_V5:public ParallelEnvironment{
    public:
      MovingBallEnvironment_V5(int robotNumber, double maxAction=5.0, double maxPosition = 2.0,double maxSpeed = 5.0, double timeStep = 0.01):
        ParallelEnvironment()
      {
      ...
      bool step(double* newState, double* reward, double* u) override{
        ...
        for(int i = 0; i<robotNumber_;i++){
          ...
          *(newState+4*i)   = *(desiredPosition_+2*i) - *(position_+2*i) + noise(generator);
          *(newState+4*i+1) = *(desiredPosition_+2*i+1) - *(position_+2*i+1) + noise(generator);
          *(newState+4*i+2) = -*(velocity_+2*i) + noise(generator);
          *(newState+4*i+3) = -*(velocity_+2*i+1) + noise(generator);

          double squareNormError = *(newState+4*i) * *(newState+4*i)
                       +*(newState+4*i+1) * *(newState+4*i+1);
      ...
      void reset(double* state) override{
        ...
        for(int i = 0; i<robotNumber_;i++){
          for(int j = 0; j<2 ;j++){
            ...
            *(velocity_+2*i+j) = 0;
            *(u_+2*i+j) = 0;
            *(state+4*i)   = *(desiredPosition_+2*i) - *(position_+2*i) + noise(generator);
            *(state+4*i+1) = *(desiredPosition_+2*i+1) - *(position_+2*i+1) + noise(generator);
            *(state+4*i+2) = -*(velocity_+2*i) + noise(generator);
            *(state+4*i+3) = -*(velocity_+2*i+1) + noise(generator);
          }
        }
