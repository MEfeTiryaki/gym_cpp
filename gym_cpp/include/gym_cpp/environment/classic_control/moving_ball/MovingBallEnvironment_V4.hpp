...
namespace gym{
  class MovingBallEnvironment_V4:public Environment{
    public:
      MovingBallEnvironment_V4( double maxAction=5.0, double maxPosition = 2.0,double maxSpeed = 5.0,double timeStep = 0.01):
        Environment()
      {
      ...
      bool step(Eigen::VectorXd& newState, double& reward, Eigen::VectorXd u) override{
        ...
        // TODO : add the noise in reward as well
        ...
        for(int i=0;i<2;i++){
          newState[2*i] += noise(generator);
          newState[2*i+1] += noise(generator);
        }

        reward = - 1.0*(newState.segment(0,2)).squaredNorm()
                 - 0.01*(u_).squaredNorm();
      ...
      Eigen::VectorXd reset() override{
        ...
        for(int i=0;i<2;i++){
          state[2*i] += noise(generator);
          state[2*i+1] += noise(generator);
        }
